...
from sklearn.metrics import pairwise_distances, accuracy_score, r2_score, average_precision_score
...
active_learning_loop  \
    ...
    --acquisition_function_path=./src/functions/BADGETeacherSelectionVote.py \
...
class BADGETeacherVote(BaseBatchAcquisitionFunction):
    teachers = []
    selections = []
    ...
    def get_targets(self, candidates):
        ...
        if len(targets[2]) > self.batch_size:
            return np.random.choice(targets[2], size=self.batch_size, replace=False)
        else:
            missing = self.batch_size - len(targets[2])
            return np.append(
                np.array(targets[2]),
